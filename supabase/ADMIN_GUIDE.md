...
Eleven tables. For product management you touch nine of them; `profiles` and
`saved_items` are user data, not catalogue.

```
GLOBAL (shared across all products)
  attributes ──── attribute_terms          e.g. Color → Oak, Walnut

CATALOGUE
  categories
      │
      ▼
  products ─────┬──── product_images                (variation_id optional)
      │         │
      │         ├──── product_attributes ──── product_attribute_terms
      │         │         (links a product to a global attribute + its terms)
      │         │
      │         └──── variations ──── variation_terms
      │                                (each term points back to attribute_terms)

USER DATA (not catalogue)
  profiles        one row per auth user, holds is_admin
  saved_items     a user's library (→ products, optionally → variations)
```

`product_attribute_terms` and `variation_terms` both point into the global
`attribute_terms` — that's how a product's options stay tied to the shared
vocabulary.
